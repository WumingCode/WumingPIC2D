...
!!************************ NUMERICAL CONSTANTS ***********************************!!
  ...
  integer, parameter :: ny     = 1020        ! NUMBER OF GRID POINTS IN Y
  ...
  integer, parameter :: np     = 80*nx     ! MAX. NUMBER OF PARTICLES IN CONUMN AT Y
...
!! SETUP FOR SUBROUTINES CALLED IN MAIN PROGRAM
  integer, parameter :: itmax  = 24000      !NUMBER OF ITERATION
  integer            :: it0    = 0         !0:INITIAL, NONZERO/9999999: RESTART DATA
  integer, parameter :: intvl1 = 2000      !INTERVAL FOR PARTICLES & FIELDS STORAGE          
  integer, parameter :: intvl2 = 1         !INTERVAL FOR INJECTING PARTICLES
  integer, parameter :: intvl3 = 2         !INTERVAL FOR EXPANDING PHYSICAL REGION IN X
...
!!   phi   : INCLINATION ANGLE FROM X-Y PLANE FOR BY & BZ
  ...
  real(8), parameter :: ma     = alpha*dsqrt(mr)*dsqrt(gam0*gam0-1.D0)/gam0
